...
import Relevance_based_Sampling.Coocurrence
import Relevance_based_Sampling.LogIndexing
import Relevance_based_Sampling.feature_encodings
from Relevance_based_Sampling.ExploreExploitDecision import should_explore_greedy
from Relevance_based_Sampling.LogIndexing import SequenceBasedLogPreprocessor
...
class GuidedLogSampler(LogSampler):
    ...
    def _prepare_sampling(self, keys, model):
        for key in keys:
            self.knowledge_base[key] = Relevance_based_Sampling.Coocurrence.coocurence_rule()
...
class FeatureGuidedLogSampler(GuidedLogSampler):
    def __init__(self, log, use_cache=True, index_file=None, verbose=False):
        super().__init__(partitioned_log={},
                         ...
                         verbose=verbose)
        self.partitioned_log, self.sample.times["partitioning"] = Relevance_based_Sampling.LogIndexing.FeatureBasedPartitioning().partition(log, index_file=index_file)
    ...
    def __assign_event_level_features(self, trace, deviation_contexts):
        ...
        for idx, is_deviating in enumerate(deviation_contexts):
            ...
            _, features = create_feature_encoding_from_index([current_event],
                                                             considered_feature_types=Relevance_based_Sampling.feature_encodings.feature_types.EVENT_LEVEL,
    ...
    def __assign_trace_level_features(self, trace, non_conformance_in_trace):
        ...
        _, features = create_feature_encoding_from_index([trace],
                                                         considered_feature_types=Relevance_based_Sampling.feature_encodings.feature_types.TRACE_LEVEL,
                                                         index_name=self.index_file)
